 ...
 * Now uses database-driven prompts via PromptService
 */

const OpenAI = require('openai');
const Anthropic = require('@anthropic-ai/sdk');
const { promptService } = require('./prompt-service.js');
...
class AIProvider {
  constructor() {
    ...
    // Reference to prompt service for database-driven prompts
  ...
  async analyzeTranscript(transcript, meetingContext = {}) {
    const prompt = await this.buildTranscriptAnalysisPrompt(transcript, meetingContext);
  ...
  async buildTranscriptAnalysisPrompt(transcript, context) {
    try {
      // Get the meeting analysis prompt from database
      const promptData = await this.promptService.getAIPrompt('meeting-analysis');
      
      if (promptData) {
        // Use database-driven prompt with variables
        const templateData = {
          company: context.company || 'Unknown',
          meetingType: context.meetingType || 'Unknown', 
          date: context.date || 'Unknown',
          participants: context.participants || 'Unknown',
          transcript: transcript
        };
        
        // Build full prompt with system instructions and user template
        const systemPrompt = promptData.system_instructions || '';
        const userPrompt = this.promptService.processTemplate(promptData.user_prompt_template, templateData);
        
        return systemPrompt ? `${systemPrompt}\n\n${userPrompt}` : userPrompt;
      }
    } catch (error) {
      console.warn('⚠️ Failed to get database prompt, using fallback:', error.message);
    }
...
`;
  ...
  async generateChangelogEntry(jiraIssue) {
    const prompt = await this.buildChangelogPrompt(jiraIssue);
  ...
  async buildChangelogPrompt(jiraIssue) {
    try {
      // Get the changelog generation prompt from database
      const promptData = await this.promptService.getAIPrompt('changelog-generation');
...
`;
  ...
  async analyzeMeetingContent(meetingData) {
    const prompt = await this.buildMeetingAnalysisPrompt(meetingData);
  ...
  async buildMeetingAnalysisPrompt(meetingData) {
    try {
      // Get the meeting analysis prompt from database (can reuse same prompt ID)
      const promptData = await this.promptService.getAIPrompt('meeting-analysis');
      
      if (promptData) {
        // Use database-driven prompt with variables
        const templateData = {
          company: meetingData.customer || 'Unknown',
          meetingType: meetingData.meetingType || 'General',
          date: new Date().toISOString().split('T')[0],
          participants: meetingData.attendees?.map(a => `${a.name} (${a.role})`).join(', ') || 'Not specified',
...
`;
  ...
  async generateEnhancedChangelogWithRelatedStories(mainStory, relatedStories) {
    const prompt = await this.buildEnhancedChangelogPrompt(mainStory, relatedStories);
  ...
  async buildEnhancedChangelogPrompt(mainStory, relatedStories) {
    try {
      // Get the changelog generation prompt from database (can reuse same prompt)
      const promptData = await this.promptService.getAIPrompt('changelog-generation');
      
      if (promptData) {
        // Build related stories context
...
- **${story.key}**: ${story.summary}
  ...
  Labels: ${story.labels.join(', ') || 'None'}`).join('\n');
        ...
        // Use database-driven prompt with enhanced context
        const templateData = {
          jiraKey: mainStory.key,
          title: mainStory.summary,
          description: `${mainStory.description}\n\nRELATED STORIES (for context):\n${relatedStoriesText}`,
          status: mainStory.status,
        ...
        // Add enhanced context instructions
        const enhancedInstructions = `\n\nENHANCED CONTEXT INSTRUCTIONS:\n1. Analyze all stories together to understand the complete scope\n2. Create a cohesive entry reflecting the broader initiative\n3. Highlight how related stories enhance the main feature\n4. Focus on combined customer value and benefits`;
        
        return systemPrompt ? `${systemPrompt}\n\n${userPrompt}${enhancedInstructions}` : `${userPrompt}${enhancedInstructions}`;
      }
    } catch (error) {
      console.warn('⚠️ Failed to get database prompt, using fallback:', error.message);
    }

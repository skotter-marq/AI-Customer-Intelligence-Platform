...
class AtlassianOAuth {
   ...
   * @param {string} accessToken - Access token
   * @param {string} cloudId - Atlassian cloud ID
   * @param {string} issueKey - Issue key (e.g., "PRESS-123")
   * @param {Object} fields - Fields to update
   * @returns {Object} Update result
   */
  async updateJiraIssue(accessToken, cloudId, issueKey, fields) {
    try {
      console.log(`🔄 OAuth: Updating JIRA issue ${issueKey} with fields:`, JSON.stringify(fields, null, 2));
      ...
      console.log(`✅ OAuth: Successfully updated JIRA issue ${issueKey}`);
      ...
      console.error(`❌ OAuth: Failed to update JIRA issue ${issueKey}:`, error.response?.data || error.message);

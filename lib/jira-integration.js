...
class JiraIntegration {
  ...
  async updateIssue(issueKey, fields) {
    try {
      ...
      console.log(`🔧 JIRA API URL: ${this.baseUrl}/rest/api/3/issue/${issueKey}`);
      console.log(`🔧 Fields to update:`, JSON.stringify(fields, null, 2));
      
      // First, check if the issue exists
      try {
        const checkResponse = await this.jiraApi.get(`/issue/${issueKey}`);
        console.log(`✅ Issue ${issueKey} exists, proceeding with update...`);
      } catch (checkError) {
        if (checkError.response?.status === 404) {
          console.error(`❌ JIRA issue ${issueKey} not found (404)`);
          return {
            success: false,
            error: `Issue ${issueKey} not found`,
            details: 'The JIRA issue does not exist or you do not have permission to view it'
          };
        }
        console.warn(`⚠️ Could not verify issue exists (continuing anyway):`, checkError.message);
      }
  ...
  async updateTLDR(issueKey, tldr) {
    try {
      console.log(`🔄 Updating TLDR for JIRA issue ${issueKey}...`);
      
      // CONFIGURE THIS: Set your actual TL;DR custom field ID here
      // To find this field ID:
      // 1. Go to a JIRA issue in your browser
      // 2. Look for the "TL;DR" field
      // 3. Inspect element or check field configuration
      // 4. Replace 'customfield_XXXXX' below with the actual field ID
      const TLDR_FIELD_ID = process.env.JIRA_TLDR_FIELD_ID || 'customfield_10087'; // TL;DR field
      
      const tldrFields = {};
      
      // Try to update the specific TL;DR custom field
      if (TLDR_FIELD_ID && TLDR_FIELD_ID !== 'unknown') {
        tldrFields[TLDR_FIELD_ID] = tldr;
        console.log(`📝 Setting ${TLDR_FIELD_ID} = "${tldr}"`);
      } else {
        // Fallback: If we don't know the field ID, don't update anything
        // This prevents overwriting important fields
        console.warn('⚠️ TL;DR field ID not configured. Set JIRA_TLDR_FIELD_ID environment variable.');
        return {
          success: false,
          error: 'TL;DR field ID not configured',
          details: 'Please set JIRA_TLDR_FIELD_ID environment variable with the correct custom field ID'
        };
      }

      // Update the issue with the TL;DR field
      const result = await this.updateIssue(issueKey, tldrFields);
      
      if (result.success) {
        console.log(`✅ Successfully updated TL;DR field for ${issueKey}`);
      } else {
        console.error(`❌ Failed to update TL;DR field for ${issueKey}:`, result.error);
      }
      
      return result;
      
    } catch (error) {
      console.error(`❌ Error updating TL;DR for ${issueKey}:`, error.message);
      return {
        success: false,
        error: error.message,
